...
float CBaseHLCombatWeapon::CalcViewmodelBob(void)
{
	...
	static float previous_vertical_offset = 0.0f;
	...
	g_verticalBob = speed * 0.002f;

	// Vertical sway using a combination of sine and cosine functions
	float verticalSwayFactor = sin(cycle) + 0.5f * cos(2.5f * cycle);
	g_verticalBob = g_verticalBob * 0.3 + g_verticalBob * 0.7 * verticalSwayFactor;

	float vertical_offset = RemapVal(speed, 0, 320, 0.0f, 16.0f);

	float lerp_amount = 0.1f;
	vertical_offset = Lerp(lerp_amount, previous_vertical_offset, vertical_offset);
	g_verticalBob -= vertical_offset;
	...
	// Lateral sway using a combination of sine and cosine functions
	float lateralSwayFactor = sin(cycle) + 0.4f * cos(3.0f * cycle);
	g_lateralBob = g_lateralBob * 0.3 + g_lateralBob * 0.7 * lateralSwayFactor;
...
void CBaseHLCombatWeapon::AddViewmodelBob(CBaseViewModel* viewmodel, Vector& origin, QAngle& angles)
{
	CBasePlayer* player = ToBasePlayer(GetOwner());
	//Assert( player );
	float velocity = player->GetAbsVelocity().Length();
	if (velocity > 1)
	{
